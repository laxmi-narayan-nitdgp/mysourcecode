...
%>
        <%
        Date date = new Date();
        String res = date.toString().substring(8,10);
   ...
   String ss =sdate.substring(0, 20);
            ...
            int xchange = Integer.parseInt(lev);
              if (xchange == check) {
                String rString = request.getParameter("ans").toLowerCase();
            ...
            sql = "update hunt_reg.registration set level='" + qid + "',date_time='"+ss+"' where email='" + session.getAttribute("username").toString() + "';";
              mdb.Transactlevel(sql);
                    x++;
                    //response.sendRedirect("/MystryHunt/question/"+p);
                    if((x<=15)&&(!res.equals("01"))){
                ...
                response.sendRedirect("/MystryHunt/question/wrong.jsp");
            }
        %>
    </body>
</html>
